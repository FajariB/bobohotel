...
class HotelRoomController extends Controller
{
    ...
    {
        //menyimpan data hotel room baru ke database dengan menggunakan transaksi database
        //closure base transaction
        DB::transaction(function() use ($request, $hotel) {
            $validated = $request->validated();

            if($request->hasFile('photo')){
                $photoPath = $request->file('photo')->store('hotel_room_photos', 'public');
    ...
    public function update(UpdateRoomRequest $request, HotelRoom $hotelRoom)
    {
        DB::transaction(function() use ($request, $hotelRoom) {
            ...
            if($request->hasFile('photo')){
                ...
                $validated['photo'] = $hotelRoom->photo;
            }
